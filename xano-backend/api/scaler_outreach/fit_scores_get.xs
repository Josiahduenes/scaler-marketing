query "fit_scores" verb=GET {
  api_group = "Scaler Outreach"
  description = "List fit scores for a workflow run"

  input {
    int workflow_run_id { table = "workflow_run" }
    int? limit?=100 filters=min:1|max:100
  }

  stack {
    function.run "security/require_service_token" {
      input = {}
    } as $auth_check

    db.query "fit_score" {
      where = $db.fit_score.workflow_run_id == $input.workflow_run_id
      sort = { created_at: "desc" }
      return = {
        type: "list",
        paging: { page: 1, per_page: $input.limit ?? 100, metadata: false }
      }
    } as $fit_scores
  }

  response = $fit_scores
  guid = "yxvc9CKv8kAGB3w_GWHzcg7PW3w"
}
